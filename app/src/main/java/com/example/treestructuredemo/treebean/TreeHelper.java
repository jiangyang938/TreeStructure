...
import com.example.treestructuredemo.R;
...
//				}
				...
				{
					f.setAccessible(true);
					label = (InfoBean) f.get(t);
					id=label.getCode();
					pId=label.getParentCode();
				}
	...
	private static void setNodeIcon(Node node)
	{
		...
		{
			node.setIcon(R.mipmap.project_closed);
		} else if (node.getChildren().size() > 0 && !node.isExpand())
		{
			node.setIcon(R.mipmap.project_spread);
		} else
			node.setIcon(-1);
